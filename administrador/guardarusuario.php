...
{
	echo '<script language="javascript">window.location="crearusuario.php?mensaje=1"</script>;';
    exit();
}
...
/*Encriptamos la contraseña de usuario*/
	...
	try {
		...
		$comprobar = $base->query("SELECT * FROM user WHERE email = '$email' AND password = '$encriptada' ");
            ...
            if ($checar != 0) {
				# code...
				echo '<script language="javascript">window.location="crearusuario.php?mensaje=2"</script>;';
				exit();
			...
			if ($ok == FALSE) {
				...
				echo "<span style='color:#ff0000'>Error en la ejecucion de la consulta</span>";
						...
						echo '<script language="javascript">window.location="crearusuario.php?mensaje=3"</script>;';
